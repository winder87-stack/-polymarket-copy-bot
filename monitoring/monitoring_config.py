...
class SecurityScanConfig(BaseModel):
    ...
    weekly_deep_scan_enabled: bool = Field(default=True, description="Enable weekly deep security scans")
    weekly_deep_scan_day: str = Field(default="sunday", description="Weekly deep scan day")

    # Scan parameters
    dependency_scan_enabled: bool = Field(default=True, description="Scan dependencies for vulnerabilities")
    ...
    critical_vulnerabilities_threshold: int = Field(default=0, description="Alert if critical vulns > threshold")
    high_vulnerabilities_threshold: int = Field(default=5, description="Alert if high vulns > threshold")
    secrets_found_threshold: int = Field(default=0, description="Alert if secrets found > threshold")
...
class PerformanceBenchmarkConfig(BaseModel):
    ...
    baseline_file: str = Field(default="monitoring/baselines/performance_baseline.json", description="Performance baseline file")
    baseline_update_threshold: float = Field(default=0.05, description="Auto-update baseline if deviation < 5%")
...
class AlertHealthConfig(BaseModel):
    ...
    min_alert_success_rate: float = Field(default=0.95, description="Minimum alert success rate (95%)")
    ...
    test_alert_message: str = Field(default="🔍 Monitoring: System health check", description="Test alert message")
...
class ReportingConfig(BaseModel):
    ...
    # Report paths
    report_directory: str = Field(default="monitoring/reports", description="Report storage directory")
...
class NotificationConfig(BaseModel):
    ...
    escalation_delays: List[int] = Field(default_factory=lambda: [300, 900, 3600], description="Escalation delays in seconds")
...
class MonitoringConfig(BaseModel):
    ...
    config_directory: str = Field(default="monitoring/config", description="Configuration directory")
...
if __name__ == "__main__":
    ...
    logger.info(f"Security scanning: {'enabled' if monitoring_config.security.daily_scan_enabled else 'disabled'}")
    logger.info(f"Performance benchmarking: {'enabled' if monitoring_config.performance.benchmark_enabled else 'disabled'}")
    logger.info(f"CI/CD pipeline: {'enabled' if monitoring_config.ci_cd.ci_enabled else 'disabled'}")
    logger.info(f"Alert health monitoring: {'enabled' if monitoring_config.alerts.health_check_enabled else 'disabled'}")
