...
class SecurityScanConfig(BaseModel):
    ...
    # Scan schedules
    daily_scan_enabled: bool = Field(
        default=True, description="Enable daily security scans"
    )
    ...
    weekly_deep_scan_day: str = Field(
        default="sunday", description="Weekly deep scan day"
    )
    ...
    secrets_scan_enabled: bool = Field(
        default=True, description="Scan for exposed secrets"
    )
    config_scan_enabled: bool = Field(
        default=True, description="Scan configuration security"
    )
    code_scan_enabled: bool = Field(
        default=True, description="Static code security analysis"
    )
...
class PerformanceBenchmarkConfig(BaseModel):
    ...
    benchmark_enabled: bool = Field(
        default=True, description="Enable performance benchmarking"
    )
    benchmark_interval_hours: int = Field(
        default=24, description="Benchmark interval in hours"
    )
    benchmark_duration_minutes: int = Field(
        default=30, description="Benchmark test duration"
    )

    # Performance thresholds
    max_memory_mb: float = Field(
        default=512.0, description="Maximum allowed memory usage"
    )
    max_cpu_percent: float = Field(
        default=80.0, description="Maximum allowed CPU usage"
    )
    max_response_time_ms: float = Field(
        default=5000, description="Maximum response time"
    )
    min_throughput_tps: float = Field(
        default=10.0, description="Minimum transactions per second"
    )

    # Benchmark scenarios
    trade_execution_benchmark: bool = Field(
        default=True, description="Benchmark trade execution"
    )
    wallet_scanning_benchmark: bool = Field(
        default=True, description="Benchmark wallet scanning"
    )
    api_call_benchmark: bool = Field(default=True, description="Benchmark API calls")
    memory_usage_benchmark: bool = Field(
        default=True, description="Benchmark memory usage"
    )
...
class CICDPipelineConfig(BaseModel):
    ...
    auto_merge_enabled: bool = Field(
        default=False, description="Enable automatic merging"
    )
    ...
    performance_tests_enabled: bool = Field(
        default=True, description="Run performance tests"
    )
    security_tests_enabled: bool = Field(default=True, description="Run security tests")

    # Test thresholds
    min_test_coverage: float = Field(
        default=85.0, description="Minimum test coverage percentage"
    )
    max_test_duration_seconds: int = Field(
        default=300, description="Maximum test duration"
    )
    allow_test_failures: bool = Field(
        default=False, description="Allow test failures in CI"
    )
...
class AlertHealthConfig(BaseModel):
    ...
    health_check_enabled: bool = Field(
        default=True, description="Enable alert health checks"
    )
    health_check_interval_minutes: int = Field(
        default=60, description="Health check interval"
    )
    alert_test_enabled: bool = Field(
        default=True, description="Send test alerts periodically"
    )


class ArbitrageMonitoringConfig(BaseModel):
    """Configuration for cross-market arbitrage monitoring"""

    # Monitoring settings
    arbitrage_monitoring_enabled: bool = Field(
        default=False, description="Enable arbitrage monitoring"
    )
    correlation_tracking_enabled: bool = Field(
        default=True, description="Track market correlation changes"
    )
    volatility_monitoring_enabled: bool = Field(
        default=True, description="Monitor market volatility"
    )

    # Alert thresholds
    alert_on_new_opportunity: bool = Field(
        default=True, description="Alert when new arbitrage opportunity detected"
    )
    alert_on_execution: bool = Field(
        default=True, description="Alert when arbitrage is executed"
    )
    alert_on_high_volatility: bool = Field(
        default=True, description="Alert when high volatility detected"
    )

    # Performance tracking
    track_pnl_per_category: bool = Field(
        default=True, description="Track P&L by market category"
    )
    track_correlation_drift: bool = Field(
        default=True, description="Track correlation drift over time"
    )
    track_edge_distribution: bool = Field(
        default=True, description="Track profit edge distribution"
    )

    # Risk monitoring
    monitor_liquidity_changes: bool = Field(
        default=True, description="Monitor liquidity changes across markets"
    )
    monitor_position_exposure: bool = Field(
        default=True, description="Monitor total arbitrage position exposure"
    )

    # Reporting
    daily_arb_report: bool = Field(
        default=True, description="Generate daily arbitrage reports"
    )
    arb_report_time: str = Field(
        default="18:00", description="Daily report time (HH:MM UTC)"
    )

    # Health monitoring
    telegram_health_check: bool = Field(
        default=True, description="Monitor Telegram bot health"
    )
    email_health_check: bool = Field(default=False, description="Monitor email alerts")
    webhook_health_check: bool = Field(
        default=False, description="Monitor webhook alerts"
    )

    # Alert thresholds
    max_alert_delay_seconds: int = Field(
        default=300, description="Maximum acceptable alert delay"
    )
    ...
    alert_failure_threshold: int = Field(
        default=3, description="Consecutive failures before alert"
    )

    # Test configurations
    test_alert_interval_hours: int = Field(
        default=24, description="Test alert interval"
    )
...
class ReportingConfig(BaseModel):
    ...
    # Report destinations
    telegram_reports: bool = Field(
        default=True, description="Send reports via Telegram"
    )
    ...
    include_security_scan: bool = Field(
        default=True, description="Include security scan results"
    )
    ...
    include_alert_health: bool = Field(
        default=True, description="Include alert health status"
    )
...
class NotificationConfig(BaseModel):
    ...
    notify_on_critical: bool = Field(
        default=True, description="Notify on critical issues"
    )
    notify_on_high: bool = Field(
        default=True, description="Notify on high-priority issues"
    )
    notify_on_medium: bool = Field(
        default=False, description="Notify on medium-priority issues"
    )
    notify_on_info: bool = Field(
        default=False, description="Notify on info-level events"
    )

    # Escalation
    escalation_enabled: bool = Field(
        default=True, description="Enable notification escalation"
    )
    escalation_delays: List[int] = Field(
        default_factory=lambda: [300, 900, 3600],
        description="Escalation delays in seconds",
    )
...
class MonitoringConfig(BaseModel):
    ...
    performance: PerformanceBenchmarkConfig = Field(
        default_factory=PerformanceBenchmarkConfig
    )
    ...
    arbitrage: ArbitrageMonitoringConfig = Field(
        default_factory=ArbitrageMonitoringConfig
    )
    ...
    monitoring_enabled: bool = Field(
        default=True, description="Master switch for monitoring"
    )
    ...
    environment: str = Field(
        default="production", description="Environment (production/staging)"
    )
    staging_mode: bool = Field(
        default=False, description="Enable staging-specific monitoring"
    )

    # Paths and directories
    base_directory: str = Field(
        default="monitoring", description="Monitoring base directory"
    )
    ...
    emergency_mode: bool = Field(
        default=False, description="Enable emergency monitoring mode"
    )
