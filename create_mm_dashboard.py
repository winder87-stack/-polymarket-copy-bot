...
def create_probability_distribution_chart(wallet_data, output_dir):
    ...
    n, bins, patches = ax.hist(probabilities, bins=20, alpha=0.7, color='#3498db', edgecolor='black')
    ...
    ax.axvline(x=0.7, color='red', linestyle='--', linewidth=2, label='Market Maker Threshold (0.7)')
    ax.axvline(x=0.4, color='orange', linestyle=':', linewidth=2, label='Mixed Trader Threshold (0.4)')

    ax.set_title('Market Maker Probability Distribution', fontsize=16, fontweight='bold')
...
def create_trading_patterns_scatter(wallet_data, output_dir):
    ...
    for i, (f, b, cls) in enumerate(zip(freq, balance, classifications)):
        ax1.scatter(f, b, color=colors.get(cls, '#95a5a6'), alpha=0.7, s=60,
                   label=cls.replace('_', ' ').title() if i == classifications.index(cls) else "")
...
def create_top_performers_table(wallet_data, output_dir):
    ...
    # Prepare table data
    headers = ['Rank', 'Wallet Address', 'Classification', 'MM Probability', 'Confidence', 'Trades/Day', 'Markets']
    table_data = []
    ...
    table = ax.table(cellText=table_data, colLabels=headers, loc='center',
                    cellLoc='center', colColours=['#f8f9fa']*len(headers))
    ...
    ax.set_title('Top 10 Wallets by Market Maker Probability', fontsize=16, fontweight='bold', pad=20)
...
"""
    ...
    for cls, count in classifications.items():
        ...
        report_content += f"| {cls.replace('_', ' ').title()} | {count} | {percentage:.1f}% |\n"
    ...
    report_content += "The following wallets show the highest market maker probabilities:\n\n"
...
"""
    ...
    report_content += "2. **Monitor** arbitrage traders (MM probability 0.6-0.7) for spread trading\n"
    ...
    report_content += "---\n"
    report_content += "*Analysis completed using advanced behavioral pattern recognition*\n"
