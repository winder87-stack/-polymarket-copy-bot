...
def create_probability_distribution_chart(wallet_data, output_dir):
    ...
    ax.axvline(
        x=0.7, color="red", linestyle="--", linewidth=2, label="Market Maker Threshold (0.7)"
    )
    ax.axvline(
        x=0.4, color="orange", linestyle=":", linewidth=2, label="Mixed Trader Threshold (0.4)"
    )
...
def create_top_performers_table(wallet_data, output_dir):
    ...
    ax.set_title(
        "Top 10 Wallets by Market Maker Probability", fontsize=16, fontweight="bold", pad=20
    )
...
"""
    ...
    report_content += (
        f"- **Average Confidence:** {summary['results_summary']['average_confidence']:.3f}\n"
    )
...
"""
    ...
    report_content += (
        "4. **Avoid** low activity wallets until they demonstrate consistent patterns\n\n"
    )
    report_content += "---\n"
