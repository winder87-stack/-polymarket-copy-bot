                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            if isinstance(data, dict) and "data" in data:
                                logger.info(
                                    f"✅ Success from {endpoint}: Retrieved {len(data['data'])} wallets"
                                )
                                return data["data"]
                            elif isinstance(data, list):
                                logger.info(
                                    f"✅ Success from {endpoint}: Retrieved {len(data)} wallets"
                                )
                                return data
...
def main():
    ...
    logger.info(
        f"🔑 API Key: {'Configured' if config.POLYMARKET_API_KEY else 'Not configured'}"
    )
    logger.info("")
    ...
    if results:
        ...
        for i, wallet in enumerate(results[:3], 1):
            ...
            logger.info(
                f"     {i}. {name} ({address}) - {trades} trades, {roi:.1f}% ROI"
            )
