...
class PolymarketLeaderboardAPI:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(f"HTTP {response.status_code} from {endpoint}: {response.text[:200]}")
                        if attempt < self.max_retries - 1:
                            time.sleep(2 ** attempt)  # Exponential backoff
                            continue

                except RequestException as e:
                    logger.error(f"Network error with {endpoint} (attempt {attempt+1}/{self.max_retries}): {str(e)}")
                    if attempt < self.max_retries - 1:
    ...
    def _get_fallback_wallets(self) -> List[Dict[str, Any]]:
        ...
        logger.info(f"✅ Using {len(fallback_wallets)} fallback wallets due to API failure")
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"API Health Check - {endpoint}: {'✅ HEALTHY' if healthy else f'❌ UNHEALTHY (Status: {response.status_code})'}")
