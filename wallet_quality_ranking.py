...
class WalletQualityRanker:
    ...
    def calculate_wallet_quality_score(
        ...
        # 4. Consistency Score
        consistency_score = self._calculate_consistency_score(volume_consistency, activity_consistency)
    ...
    def _calculate_consistency_score(self, volume_consistency: float, activity_consistency: float) -> float:
    ...
    def _calculate_behavioral_stability_score(self, metrics: Dict[str, Any], category: str) -> float:
    ...
    def _calculate_confidence_score(self, wallet_data: Dict[str, Any], metrics: Dict[str, Any]) -> float:
    ...
    def rank_wallets_by_category(self, market_maker_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for category, wallets in category_wallets.items():
            ...
            category_rankings[category] = {
                ...
                "average_score": np.mean([w['overall_quality_score'] for w in ranked_wallets]) if ranked_wallets else 0,
        ...
        return {
            ...
            "total_wallets_ranked": sum(len(cat['ranked_wallets']) for cat in category_rankings.values())
        }
    ...
    def _generate_ranking_summary(self, category_rankings: Dict[str, Any], mm_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary['recommendations'] = self._generate_investment_recommendations(summary, category_rankings)

        return summary

    def _generate_investment_recommendations(self, summary: Dict[str, Any], category_rankings: Dict[str, Any]) -> List[str]:
        ...
        if elite_count > 0:
            recommendations.append(f"🎯 PRIORITY: {elite_count} Elite-quality wallets identified for immediate copy trading")

        # Category-specific recommendations
        for category, leader in summary['category_leaders'].items():
            if leader and leader['overall_quality_score'] >= 75:
                category_name = category.replace('_', ' ').title()
                recommendations.append(f"🏆 Top {category_name}: {leader['wallet_address'][:12]}... (Score: {leader['overall_quality_score']:.1f})")

        # Risk diversification recommendations
        category_count = len([c for c in category_rankings.keys() if category_rankings[c]['wallet_count'] >= 3])
        if category_count >= 3:
            recommendations.append(f"📊 DIVERSIFICATION: Strong performers available across {category_count} categories for balanced portfolio")

        # Low quality warnings
        poor_performers = summary['quality_tier_distribution'].get('Poor', 0) + summary['quality_tier_distribution'].get('Low', 0)
        if poor_performers > 0:
            recommendations.append(f"⚠️ CAUTION: {poor_performers} low-quality wallets identified - avoid for primary trading")
...
def main():
    ...
    if not results_file.exists():
        logger.error("❌ Market maker detection results not found. Run market maker detection first.")
        return 1
    ...
    for category, leader in ranking_results['summary_statistics']['category_leaders'].items():
        if leader:
            print(f"   {category.replace('_', ' ').title()}: {leader['wallet_address'][:12]}... ({leader['overall_quality_score']:.1f})")
