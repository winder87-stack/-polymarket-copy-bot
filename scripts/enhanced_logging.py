...
class EnhancedLoggingSystem:
    ...
    def _setup_loggers(self, log_level: str) -> None:
        ...
        self.app_logger = self._create_structured_logger(
            "polymarket.app", numeric_level, "app.log", use_json=True
        )

        # Trade logger
        self.trade_logger = self._create_structured_logger(
            "polymarket.trade", numeric_level, "trade.log", use_json=True
        )

        # System logger
        self.system_logger = self._create_structured_logger(
            "polymarket.system", numeric_level, "system.log", use_json=False
        )
        ...
        self.performance_logger = self._create_structured_logger(
            "polymarket.performance", numeric_level, "performance.log", use_json=True
        )

        # Error logger (separate for critical errors)
        self.error_logger = self._create_structured_logger(
            "polymarket.error", logging.ERROR, "errors.log", use_json=True
        )
    ...
    def get_log_statistics(self) -> Dict[str, Any]:
        ...
        for log_file in self.log_dir.glob("*.log*"):
            try:
                ...
                if (
                    stats[base_name]["last_modified"] is None
                    or stat_info.st_mtime > stats[base_name]["last_modified"]
                ):
        ...
        for logger_stats in stats.values():
            if logger_stats["last_modified"]:
                logger_stats["last_modified"] = datetime.fromtimestamp(
                    logger_stats["last_modified"]
                ).isoformat()
                        ...
                        try:
                            if line.strip().startswith("{"):
                                ...
                                entry_time = datetime.fromisoformat(
                                    log_entry.get("timestamp", "2000-01-01T00:00:00")
                                )
...
def main():
    ...
    if args.action == "setup":
        ...
        results = logging_system.search_logs(args.query or "", args.logger, args.hours)
        ...
        if success:
            ...
            print("❌ Failed to change log level")
