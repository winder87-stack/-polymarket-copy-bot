...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/staging_config.log'),
...
class RiskManagementConfig(BaseModel):
    max_position_size: float = Field(default=5.0, description="Maximum USDC per position (staging: $5)", ge=1.0)
    max_daily_loss: float = Field(default=10.0, description="Circuit breaker threshold (staging: $10)", ge=0.0)
    min_trade_amount: float = Field(default=0.5, description="Ignore trades smaller than this (staging: $0.50)", ge=0.01)
    max_concurrent_positions: int = Field(default=3, description="Maximum open positions (staging: 3)", ge=1)
    stop_loss_percentage: float = Field(default=0.10, description="10% stop loss (staging: more conservative)", ge=0.01, le=0.5)
    take_profit_percentage: float = Field(default=0.15, description="15% take profit (staging: more conservative)", ge=0.01, le=1.0)
    max_slippage: float = Field(default=0.05, description="5% maximum slippage (staging: higher tolerance)", ge=0.001, le=0.1)

class NetworkConfig(BaseModel):
    clob_host: str = Field(default="https://clob.polymarket.com", description="Polymarket CLOB host")
    chain_id: int = Field(default=80001, description="Polygon Mumbai testnet")
    polygon_rpc_url: str = Field(default="https://rpc-mumbai.maticvigil.com", description="Polygon Mumbai testnet RPC")
...
class TradingConfig(BaseModel):
    ...
    wallet_address: Optional[str] = Field(None, description="Wallet address (auto-calculated if empty)")
    gas_limit: int = Field(default=300000, description="Default gas limit", ge=21000)
    max_gas_price: int = Field(default=50, description="Maximum gas price in gwei (staging: lower)", ge=1)
    gas_price_multiplier: float = Field(default=1.2, description="Gas price multiplier for priority", ge=1.0, le=2.0)

class MonitoringConfig(BaseModel):
    monitor_interval: int = Field(default=30, description="Seconds between wallet checks (staging: slower)", ge=5, le=300)
    wallets_file: str = Field(default="config/wallets_staging.json", description="Path to staging wallets config")
    target_wallets: List[str] = Field(default_factory=list, description="List of wallet addresses to monitor")
    min_confidence_score: float = Field(default=0.8, description="Minimum confidence score (staging: higher threshold)", ge=0.1, le=0.95)

class AlertingConfig(BaseModel):
    telegram_bot_token: Optional[str] = Field(None, description="Telegram bot token (staging channel)")
    ...
    alert_on_circuit_breaker: bool = Field(default=True, description="Send alerts when circuit breaker activates")
...
class LoggingConfig(BaseModel):
    ...
    log_file: str = Field(default="logs/polymarket_bot_staging.log", description="Staging log file path")
    log_format: str = Field(default="%(asctime)s [STAGING] - %(name)s - %(levelname)s - %(message)s", description="Log format with staging indicator")

class StagingConfig(BaseModel):
    """Additional staging-specific configuration"""
    environment: str = Field(default="staging", description="Environment identifier")
    testnet_faucet_url: str = Field(default="https://faucet.polygon.technology", description="Testnet faucet URL")
    ...
    enable_extended_logging: bool = Field(default=True, description="Enable detailed logging for debugging")
...
class Settings(BaseModel):
    ...
    def validate_trading_config(cls, v):
        if not v.get('private_key'):
            ...
            if not private_key:
                raise ValueError("STAGING_PRIVATE_KEY must be set in environment variables for staging")
        ...
        if os.path.exists(wallets_file):
            try:
                with open(wallets_file, 'r') as f:
                    ...
                    logger.info(f"✅ Staging: Loaded {len(v['target_wallets'])} target wallets from {wallets_file}")
    ...
    def load_from_env(cls, values):
        ...
        for model_path, env_var in cls.env_mappings.items():
            ...
            if env_value is not None:
                ...
                final_key = keys[-1]
                if final_key in ['chain_id', 'gas_limit', 'max_gas_price', 'monitor_interval', 'max_concurrent_positions', 'max_trades_per_day']:
                    try:
                        current[final_key] = int(env_value)
                    except ValueError:
                        pass
                elif final_key in ['max_slippage', 'max_position_size', 'max_daily_loss', 'min_trade_amount',
                                 'stop_loss_percentage', 'take_profit_percentage', 'gas_price_multiplier', 'min_confidence_score']:
                    try:
                        current[final_key] = float(env_value)
                    except ValueError:
                        pass
                elif final_key in ['alert_on_trade', 'alert_on_error', 'alert_on_circuit_breaker', 'enable_paper_trading', 'enable_extended_logging', 'alert_on_all_events']:
    ...
    def validate_critical_settings(self):
        ...
        if len(self.monitoring.target_wallets) == 0:
            logger.warning("⚠️ Staging: No target wallets configured. Bot will run but not copy any trades.")
        ...
        if self.network.chain_id != 80001:
            logger.warning(f"⚠️ Staging: Using chain_id {self.network.chain_id}, expected Mumbai testnet (80001)")
        ...
        try:
            ...
            if not web3.is_connected():
                logger.warning("❌ Staging: Could not connect to Polygon Mumbai testnet RPC endpoint.")
            else:
        ...
        logger.info(f"   Risk parameters: Max position=${self.risk.max_position_size:.2f}, Max daily loss=${self.risk.max_daily_loss:.2f}")
        logger.info(f"   Monitoring {len(self.monitoring.target_wallets)} wallets every {self.monitoring.monitor_interval} seconds")
        logger.info(f"   Environment: {self.staging.environment}, Max trades/day: {self.staging.max_trades_per_day}")
        logger.info(f"   Paper trading: {'ENABLED' if self.staging.enable_paper_trading else 'DISABLED'}")
...
logger.info(f"Network: {staging_settings.network.polygon_rpc_url} (Chain ID: {staging_settings.network.chain_id})")
logger.info(f"Risk management: Max daily loss=${staging_settings.risk.max_daily_loss:.2f}")
logger.info(f"🚨 STAGING ENVIRONMENT - ALL TRADES ARE TESTNET ONLY 🚨")

if __name__ == "__main__":
    staging_settings.validate_critical_settings()
    print("✅ Staging configuration validated successfully")
    print(f"🚨 STAGING ENVIRONMENT - TESTNET ONLY 🚨")
