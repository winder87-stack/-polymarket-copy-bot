...
class PerformanceAnalyzer:
    ...
    async def record_trade_performance(
        self,
        ...
        logger.debug(f"📊 Recorded trade performance for {trade_record.get('wallet_address', 'unknown')[:8]}...")
    ...
    def _calculate_type_performance_metrics(self, type_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        profit_factor = (type_data[type_data["pnl_usd"] > 0]["pnl_usd"].sum() /
                        abs(type_data[type_data["pnl_usd"] < 0]["pnl_usd"].sum())) if losing_trades > 0 else float('inf')
        ...
        if "daily_return" in type_data.columns:
            ...
            if len(daily_returns) > 1:
                ...
                sharpe_ratio = (avg_daily_return - self.analysis_config["risk_free_rate"] / 365) / std_daily_return if std_daily_return > 0 else 0
            else:
                sharpe_ratio = 0
        else:
            sharpe_ratio = 0
        ...
        if len(pnl_values) > 1:
            ...
            if len(downside_returns) > 0:
                ...
                sortino_ratio = (avg_return - self.analysis_config["risk_free_rate"] / 365) / downside_deviation if downside_deviation > 0 else 0
            else:
        ...
        if max_drawdown > 0:
            ...
            days_traded = (type_data["timestamp"].max() - type_data["timestamp"].min()).days if len(type_data) > 1 else 1
        ...
        # Gas efficiency
        avg_gas_cost = type_data["gas_cost_usd"].mean() if "gas_cost_usd" in type_data.columns else 0
    ...
    def _calculate_comparative_statistics(self, wallet_type_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric in ["win_rate", "total_pnl", "sharpe_ratio", "profit_factor"]:
            ...
            rankings[metric] = {wallet_type: rank + 1 for rank, wallet_type in enumerate(sorted_types)}

        return {
            "performance_rankings": rankings,
            "statistical_tests": statistical_tests,
            "best_performing": {
                "highest_win_rate": max(comparison_data.keys(), key=lambda x: comparison_data[x]["win_rate"]),
                "highest_total_pnl": max(comparison_data.keys(), key=lambda x: comparison_data[x]["total_pnl"]),
                "highest_sharpe_ratio": max(comparison_data.keys(), key=lambda x: comparison_data[x]["sharpe_ratio"]),
                "highest_profit_factor": max(comparison_data.keys(), key=lambda x: comparison_data[x]["profit_factor"])
            }
        }
        ...
        if len(sorted_types) >= 2:
            ...
            return {
                "top_two_comparison": {
                    ...
                    "t_statistic": t_stat,
                    "p_value": p_value,
                    "statistically_significant": p_value < (1 - self.analysis_config["confidence_level"])
                }
            }
    ...
    def generate_performance_report(
        ...
        if report_type in ["detailed", "comprehensive"]:
            ...
            report["attribution_analysis"] = self.calculate_attribution_analysis(start_date, end_date)
    ...
    def calculate_attribution_analysis(
        ...
        profit_attribution = df.groupby("wallet_type")["pnl_usd"].agg([
            "sum", "mean", "count", lambda x: (x > 0).sum()
        ]).round(2)

        profit_attribution.columns = ["total_pnl", "avg_pnl", "trade_count", "winning_trades"]
        profit_attribution["win_rate"] = (profit_attribution["winning_trades"] / profit_attribution["trade_count"]).round(3)

        # Risk contribution
        risk_contribution = df.groupby("wallet_type")["pnl_usd"].agg([
            "std", "min", lambda x: np.percentile(x, 5)
        ]).round(2)
    ...
    def perform_risk_analysis(
        ...
        for wallet_type in df["wallet_type"].unique():
            ...
            risk_by_type[wallet_type] = {
                ...
                "contribution_to_portfolio_volatility": np.std(type_returns) / risk_metrics["portfolio_volatility"]
            }
    ...
    def generate_optimization_recommendations(self) -> Dict[str, Any]:
        ...
        for wallet_type, metrics in wallet_metrics.items():
            ...
            profit_factor_score = min(metrics["profit_loss_metrics"]["profit_factor"] / 3, 1.0)  # Cap at 3
        ...
        sorted_types = sorted(performance_scores.keys(), key=lambda x: performance_scores[x], reverse=True)
        ...
        recommendations["underperforming_types"] = [
            wallet_type for wallet_type, score in performance_scores.items()
    ...
    def _brinson_attribution_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        type_performance = df.groupby("wallet_type").agg({
            "pnl_usd": ["sum", "count"],
            "position_size_usd": "mean"
        }).round(2)

        type_performance.columns = ["total_pnl", "trade_count", "avg_position_size"]
        type_performance["pnl_per_trade"] = type_performance["total_pnl"] / type_performance["trade_count"]
        ...
        return {
            ...
            "unattributed_return": type_performance["total_pnl"].sum() - sum([attr["total_attribution"] for attr in attribution.values()])
        }
    ...
    def _factor_attribution_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "market_conditions" in df.columns:
            ...
            for col in market_data.select_dtypes(include=[np.number]).columns:
                if market_data[col].std() > 0:  # Only for variable factors
                    factor_return = np.corrcoef(market_data[col], df["pnl_usd"])[0, 1] * df["pnl_usd"].std()
        ...
        return {
            ...
            "top_factors": sorted(factor_contributions.items(), key=lambda x: abs(x[1]), reverse=True)[:5]
        }
    ...
    def _holdings_based_attribution(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for wallet_type in daily_performance.columns:
            ...
            attribution[f"{wallet_type}_allocation"] = (
                benchmark_weights[wallet_type] * (daily_performance[wallet_type] - daily_performance.mean(axis=1))
            )
    ...
    def calculate_correlation_matrix(
            ...
            for j, wallet_type2 in enumerate(corr_matrix.columns):
                if i < j:  # Only upper triangle
                    ...
                    correlation_stats[f"{wallet_type1}_vs_{wallet_type2}"] = {
                        ...
                        "strength": "very_strong" if abs(corr_value) > 0.8 else
                                   "strong" if abs(corr_value) > 0.6 else
                                   "moderate" if abs(corr_value) > 0.4 else
                                   "weak" if abs(corr_value) > 0.2 else "very_weak",
        ...
        return {
            ...
            "correlation_diversity_score": 1 - abs(corr_matrix.values[np.triu_indices_from(corr_matrix.values, k=1)]).mean()
        }
    ...
    def analyze_opportunity_cost(
        ...
        avg_daily_opportunity_cost = np.mean(np.array(optimal_daily_returns) - np.array(actual_daily_returns))

        # Days where optimal choice would have helped
        profitable_deviations = sum(1 for opt, act in zip(optimal_daily_returns, actual_daily_returns) if opt > act)
        ...
        return {
            ...
            "opportunity_cost_ratio": total_opportunity_cost / abs(actual_cumulative[-1]) if actual_cumulative[-1] != 0 else float('inf'),
            "best_wallet_type_days": daily_performance.idxmax(axis=1).value_counts().to_dict(),
            "recommendation": "High opportunity cost suggests dynamic wallet selection could significantly improve returns" if abs(total_opportunity_cost) > abs(actual_cumulative[-1]) * 0.2 else "Opportunity cost is reasonable - current strategy is effective"
        }
    ...
    def create_benchmark_portfolios(
        ...
        benchmarks["equal_weight"] = self._calculate_portfolio_metrics_from_returns(equal_weight_returns)

        # Risk Parity Portfolio
        risk_parity_returns = self._simulate_risk_parity_portfolio(df)
        benchmarks["risk_parity"] = self._calculate_portfolio_metrics_from_returns(risk_parity_returns)
    ...
    def _compare_benchmarks(self, benchmarks: Dict[str, Any]) -> Dict[str, Any]:
        ...
        comparison["key_differences"] = {
            ...
            "best_vs_worst_return_ratio": max(returns) / min(returns) if min(returns) != 0 else float('inf'),
            "consistent_performers": sum(1 for bench in benchmarks.values() if bench["total_return"] > 0)
        }
    ...
    def _generate_benchmark_recommendations(self, comparison: Dict[str, Any]) -> List[str]:
        ...
        if perf_rankings and risk_rankings:
            ...
            for strategy in perf_rankings.keys():
                combined_rank = perf_rankings[strategy] + risk_rankings.get(strategy, len(risk_rankings))
                if combined_rank < best_score:
                    best_score = combined_rank
                    best_hybrid = strategy

            if best_hybrid:
                recommendations.append(f"🏆 **Best Overall Strategy**: {best_hybrid.replace('_', ' ').title()} performs well in both return and risk-adjusted metrics")

        # Specific recommendations
        if "market_maker_only" in perf_rankings and perf_rankings["market_maker_only"] <= 2:
            recommendations.append("🎯 **Market Maker Focus**: Pure market maker strategy shows strong performance - consider increasing MM allocation")

        if "risk_parity" in risk_rankings and risk_rankings["risk_parity"] <= 2:
            recommendations.append("⚖️ **Risk Parity Approach**: Risk parity outperforms in risk-adjusted metrics - consider volatility-based allocation")

        key_diffs = comparison.get("key_differences", {})
        if key_diffs.get("return_range", 0) > 1000:  # Large performance differences
            recommendations.append("📊 **Strategy Selection Critical**: Large performance differences between strategies suggest opportunity for optimization")
    ...
    def save_performance_data(self):
        ...
        try:
            ...
            with open(performance_file, 'w', encoding='utf-8') as f:
                json.dump(self.performance_data[-1000:], f, indent=2, default=str)  # Last 1000 records
    ...
    async def run_automated_analysis(self):
        ...
        if "wallet_type_performance" in report:
            ...
            if "comparative_statistics" in perf_data:
                best_performer = perf_data["comparative_statistics"]["best_performing"]["highest_total_pnl"]
