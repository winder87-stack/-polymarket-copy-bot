...
class MarketMakerBacktester:
    ...
    async def run_comprehensive_backtest(
        ...
        logger.info(f"🧪 Starting comprehensive backtest: {test_data_type} data, {test_period_days} days, {len(strategies_to_test)} strategies")
    ...
    async def _generate_synthetic_test_data(self, period_days: int) -> List[Dict[str, Any]]:
        ...
        for wallet_type, params in wallet_types.items():
            ...
            for i in range(trade_count):
                ...
                if is_win:
                    pnl_pct = abs(random.gauss(params["avg_profit_pct"], params["avg_profit_pct"] * 0.5))
                else:
                    pnl_pct = -abs(random.gauss(params["avg_loss_pct"], abs(params["avg_loss_pct"]) * 0.5))
                ...
                trade = {
                    ...
                    "holding_time_hours": random.gauss(params["holding_time_hours"], params["holding_time_hours"] * 0.5),
        ...
        logger.info(f"🎲 Generated {len(test_data)} synthetic trades across {len(wallet_types)} wallet types")
        return test_data
    ...
    def _calculate_strategy_metrics(self, portfolio: Dict[str, Any], strategy_name: str) -> Dict[str, Any]:
        ...
        rejection_rate = len(rejected_trades) / len(portfolio["trade_history"]) if portfolio["trade_history"] else 0
    ...
    def _compare_strategy_performance(self, strategy_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(strategy_results) >= 2:
            returns = [result["performance_metrics"]["total_return_pct"] for result in strategy_results.values()]
    ...
    async def _run_robustness_tests(
        ...
        original_test_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for strategy_name in strategy_results.keys():
            ...
            for run in range(self.backtest_config["monte_carlo_runs"]):
                ...
                mc_result = await self._run_strategy_backtest(strategy_name, shuffled_data[:len(shuffled_data)//2])
        ...
        walk_forward_results = await self._run_walk_forward_analysis(strategy_results, original_test_data)
    ...
    async def _run_stress_tests(
        ...
        stress_scenarios = {
            "high_volatility": lambda trade: {**trade, "volatility_index": trade.get("volatility_index", 1.0) * 2.0},
            "low_liquidity": lambda trade: {**trade, "market_liquidity_score": trade.get("market_liquidity_score", 1.0) * 0.3},
            "high_gas": lambda trade: {**trade, "gas_price": trade.get("gas_price", 100) * 3.0},
            "adverse_pnl": lambda trade: {**trade, "actual_pnl_pct": trade.get("actual_pnl_pct", 0) * 1.5}  # Make losses worse
        }
    ...
    def _generate_backtest_recommendations(self, comparison_results: Dict[str, Any]) -> List[str]:
        ...
        recommendations.append(f"🎯 **Recommended Strategy**: {best_strategy} (best performer in backtest)")
        ...
        if comparison_results.get("risk_adjusted_rankings"):
            ...
            if risk_best != best_strategy:
                recommendations.append(f"⚖️ **Risk-Adjusted Alternative**: Consider {risk_best} for better risk-adjusted returns")
        ...
        if comparison_results.get("statistical_significance"):
            ...
            if stats.get("consistent_winners", 0) >= len(rankings) * 0.7:
                recommendations.append("✅ **Robust Results**: Most strategies show positive returns")
            else:
                recommendations.append("⚠️ **Variable Results**: Strategy performance varies significantly")

        # Key differences insights
        differences = comparison_results.get("key_differences", {})
        if differences.get("win_rate_range", 0) > 0.2:
            recommendations.append("📊 **Win Rate Variation**: Large differences in win rates suggest strategy-specific market fit")
    ...
    async def run_parameter_optimization(
        ...
        for combo in combinations:
            ...
            strategy_manager = self._create_optimized_strategy(strategy_name, params)
        ...
        best_result = max(optimization_results, key=lambda x: x["performance_metrics"]["sharpe_ratio"])

        # Analyze parameter sensitivity
        sensitivity_analysis = self._analyze_parameter_sensitivity(optimization_results, param_names)
    ...
    def _create_optimized_strategy(self, strategy_name: str, params: Dict[str, float]) -> MarketMakerRiskManager:
        ...
        if "position_size_multiplier" in params:
            strategy_manager.wallet_type_configs["market_maker"]["position_size_multiplier"] = params["position_size_multiplier"]

        if "stop_loss_pct" in params:
            strategy_manager.wallet_type_configs["market_maker"]["stop_loss_pct"] = params["stop_loss_pct"]

        if "take_profit_pct" in params:
            strategy_manager.wallet_type_configs["market_maker"]["take_profit_pct"] = params["take_profit_pct"]

        if "min_trade_quality_score" in params:
            strategy_manager.wallet_type_configs["market_maker"]["min_trade_quality_score"] = params["min_trade_quality_score"]
        ...
        for param_name in param_names:
            ...
            if len(set(param_values)) > 1:  # Only if parameter actually varies
                ...
                sensitivity[param_name] = {
                    ...
                    "impact_level": "high" if abs(correlation) > 0.7 else "medium" if abs(correlation) > 0.3 else "low"
                }
