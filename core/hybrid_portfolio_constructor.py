...
class HybridPortfolioConstructor:
    ...
    async def construct_optimal_portfolio(
        ...
        if allocation_method == "risk_parity":
            ...
            allocation = await self._performance_weighted_allocation(valid_wallets, performance_data)
        elif allocation_method == "correlation_diversified":
            allocation = await self._correlation_diversified_allocation(valid_wallets, performance_data)
        ...
        logger.info(f"✅ Portfolio constructed using {allocation_method}: {len(constrained_allocation)} allocations")
    ...
    async def _filter_and_validate_wallets(
        ...
        for wallet in wallets:
            ...
            wallet["validation_score"] = self._calculate_wallet_validation_score(wallet_info, recent_trades)
    ...
    async def _calculate_wallet_historical_metrics(self, wallet_address: str) -> Dict[str, Any]:
        ...
        wallet_trades = [
            t for t in trades
            if t.get("wallet_address") == wallet_address and
            datetime.fromisoformat(t["timestamp"]) > datetime.now() - timedelta(days=30)
        ]
    ...
    async def _calculate_wallet_correlations(self, wallets: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    async def _correlation_diversified_allocation(
        ...
        correlations = performance_data["correlations"]
    ...
    async def _hybrid_optimization_allocation(
        ...
        performance_weighted = await self._performance_weighted_allocation(wallets, performance_data)
        correlation_diversified = await self._correlation_diversified_allocation(wallets, performance_data)
        ...
        for wallet in wallets:
            ...
            combined_weight = (
                ...
                performance_weighted.get(wallet_address, 0) * hybrid_weights["performance_weighted"] +
                correlation_diversified.get(wallet_address, 0) * hybrid_weights["correlation_diversified"]
            )
    ...
    def _apply_allocation_constraints(
        ...
        max_drawdown_limit = constraints.get("max_drawdown_limit",
                                           self.construction_params["max_drawdown_limit"])
    ...
    async def _calculate_portfolio_risk_metrics(
        ...
        volatilities = [performance_data["volatilities"].get(wallet, 0.2) for wallet in allocation.keys()]
    ...
    def _generate_allocation_summary(
        ...
        for wallet in wallets:
            ...
            allocation_pct = allocated_capital / portfolio_metrics["total_allocated_capital"] if portfolio_metrics["total_allocated_capital"] > 0 else 0
    ...
    def _assess_portfolio_risk(self, portfolio_metrics: Dict[str, Any]) -> Dict[str, str]:
        ...
        avg_risk_score = np.mean([
            ...
            risk_scores.get("diversification_quality", "medium") == "low" and 3 or
            risk_scores.get("diversification_quality", "medium") == "high" and 1 or 2
        ])
    ...
    async def run_stress_tests(
        ...
        if scenario_results:
            ...
            stress_results["average_drawdown"] = np.mean([r["max_drawdown"] for r in scenario_results])
        ...
        logger.info(f"🧪 Stress testing completed: {stress_results['portfolio_survival_rate']:.1%} survival rate")
    ...
    async def run_scenario_analysis(
        ...
        return {
            "base_allocation_performance": await self._calculate_allocation_performance(portfolio_allocation),
            "scenario_analysis": scenario_results,
            "most_stressful_scenario": min(scenario_results.keys(),
                                        key=lambda x: scenario_results[x]["sharpe_ratio"]),
            "most_opportunistic_scenario": max(scenario_results.keys(),
    ...
    async def _calculate_allocation_performance(self, allocation: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _get_wallet_recent_activity(self, wallet_address: str, days: int) -> List[Dict[str, Any]]:
        """Get recent trading activity for a wallet."""

        cutoff_date = datetime.now() - timedelta(days=days)

        recent_trades = [
            trade for trade in self.analyzer.performance_data
            if (trade.get("wallet_address") == wallet_address and
    ...
    def get_portfolio_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_rebalance": self.portfolio_history[-1]["timestamp"] if self.portfolio_history else None,
    ...
    def save_portfolio_state(self):
        ...
        try:
            ...
            with open(state_dir / "portfolio_history.json", 'w') as f:
                json.dump(self.portfolio_history[-100:], f, indent=2, default=str)  # Last 100 records
