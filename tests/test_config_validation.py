...
from config.settings import Settings, TradingConfig, MonitoringConfig, NetworkConfig, RiskManagementConfig
...
class TestTradingConfigValidation:
    ...
    def test_invalid_private_key_format(self):
        ...
        for invalid_key in invalid_keys:
            with patch.dict(os.environ, {'PRIVATE_KEY': invalid_key}):
                with pytest.raises(ValueError, match="Private key must start with '0x' and be 66 characters long"):
                    TradingConfig()
    ...
    def test_missing_private_key(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError, match="PRIVATE_KEY must be set in environment variables"):
                TradingConfig()
...
class TestValidatorSignatures:
    ...
    def test_validator_decorators(self):
        ...
        assert hasattr(Settings.validate_trading_config, '__validator_config__') or \
               hasattr(Settings, '__pydantic_validator__')
